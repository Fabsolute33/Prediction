        ...
        <div className="w-full max-w-4xl mx-auto">
            ...
            <div className="md:hidden flex flex-col gap-4">
                {draws.map((draw, idx) => {
                    const predNumbers = draw.prediction?.numbers || [];
                    return (
                        <div key={draw.id} className={`glass-card p-4 ${idx === 0 ? 'border-l-4 border-l-blue-500' : ''}`}>
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-xs font-mono text-gray-400">#{draw.draw_id}</span>
                                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase ${draw.source === 'scrape' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-gray-700/50 text-gray-500'}`}>
                                            {draw.source === 'scrape' ? 'LIVE' : 'CSV'}
                                        </span>
                                    </div>
